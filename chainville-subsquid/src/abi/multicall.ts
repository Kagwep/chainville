...
const aggregate = fun('aggregate', '0x252dba42', {
...
const tryAggregate = fun('tryAggregate', '0xbce38bd7', {
...
export class Multicall extends ContractBase {
  ...
  async aggregate(...args: any[]): Promise<any[]> {
    ...
    for (let [from, to] of splitIntoPages(size, page)) {
      let result = await this.eth_call(aggregate, {calls: calls.slice(from, to)})
